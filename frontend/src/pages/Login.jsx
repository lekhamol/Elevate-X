...
import { ShieldCheck, Activity, KeyRound, UserCheck, Sparkles, ArrowRight } from 'lucide-react';
...
export default function Login() {
  ...
  const handleQuickDemo = (selectedRole) => {
    login(`${selectedRole.toLowerCase()}@safetytwin.io`, 'demo123', selectedRole);
    navigate('/dashboard');
  };
      ...
      <div className="w-full max-w-md space-y-6 relative z-10">
        ...
        <div className="glass-panel p-8 rounded-2xl shadow-2xl border border-slate-800">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              className="w-full mt-2 py-3 px-4 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 text-slate-950 font-bold text-xs hover:from-cyan-400 hover:to-blue-500 shadow-lg shadow-cyan-500/20 flex items-center justify-center space-x-2 transition-all"
            >
          ...
          <div className="mt-6 pt-6 border-t border-slate-800/80 text-center space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className="px-3 py-2 rounded-lg bg-slate-900/80 hover:bg-slate-800 border border-slate-800 text-[11px] text-cyan-300 font-mono transition-colors"
              >
                Lead Engineer
              </button>
              <button
                onClick={() => handleQuickDemo('ADMIN')}
                className="px-3 py-2 rounded-lg bg-slate-900/80 hover:bg-slate-800 border border-slate-800 text-[11px] text-blue-300 font-mono transition-colors"
              >
                System Admin
              </button>
            </div>
